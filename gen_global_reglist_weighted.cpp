...
vector<string> SplitFilename (const std::string& str) {
  vector<string> str_idx;
  //std::cout << "Splitting: " << str << '\n';
  std::size_t found = str.find_last_of("/.");
  str_idx.push_back(str.substr(0, found));
  str_idx.push_back(str.substr(found+1));
  return str_idx;
}
...
int main(int argc, char * argv[]){
    ...
    strcat(buffer, "/str_idx_final_map.log");
    char* str_idx_file = buffer;
    bool file_flag = 0;
    int option = 0;
    ...
    size_t str_idx_line_size = 0;   //indicate that getline should d-allocate space for line
    string cur_name;
    ...
    size_t line_length = 0;    //length of line from getline()
    ...
    while((line_len = getline(&line, &line_size, header_ptr) != -1)){
        ...
        if((line[0] == '\n') || (line[0] == '#')) continue; //skip newline
            ...
            struct_temp.weight.clear();
            struct_temp.index.clear();
        ...
        //is a UInt
            ...
            struct_temp.weight.push_back(1);
            strcpy(line_backup, line_backup_temp);  //working with C strings is suffering
            //---------------------------------------------------------
            ...
            FILE * str_idx_ptr = fopen(str_idx_file,"r");
            if(str_idx_ptr == NULL) {
                printf("\nCould not open string index map file: %s\n", str_idx_file);
                return 3;
            }
            vector<string> cur_uint_name_split = SplitFilename(cur_uint_name);
            cout<<"\nCur UInt name is: "<<cur_uint_name;
            cout<<"\nAfter Split "<<cur_uint_name_split[0]<<" "<<cur_uint_name_split[1];
            bool flag = false;
            while((line_length = getline(&str_idx_line, &str_idx_line_size, str_idx_ptr) != -1)) {
                string token = strtok(str_idx_line, " ");
                string index = strtok(NULL, " ");
                printf("Token is: %s", str_idx_line);

                if (token == cur_uint_name) { 
                    struct_temp.index.push_back(index);
                    flag = true;
                    break;
                }
                else if(token == cur_uint_name_split[1]) {
                    struct_temp.index.push_back(index);
                    flag = true;
                    break;
                }
            ...
            if(flag == false) 
                struct_temp.index.push_back("9999999");
