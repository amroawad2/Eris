...
/*vector<long> autocorrect;
void add_correction_indices() {
    ifstream fin;
    fin.open("autocorrect_indices.log");
    if(!fin) {
        std::cout<<"\n Unable to open autocorrect indices log. Exitting!";
        exit(0);
    }
    unsigned long idx;
    while (fin >> idx) {
        autocorrect.push_back(idx);
    }
    fin.close();
}*/
...
void load_faults(unsigned long idx, int start_cycle = -1, int end_cycle = -1); //declaration
...
#define TWO_ARG_CHECK_cat() {                                                               \
    ...
    if(to_return.meta.faulty_new) {                                                            \
        ...
        /*if(find(autocorrect.begin(), autocorrect.end(), to_return.meta.index) != autocorrect.end()) {  \
            for(int i = 0; i < n_; i++)                                                             \
                to_return.words_[i] = to_return.Origdata[i];                                              \
            to_return.meta.faulty_new = 0;                                                             \
            to_return.meta.faulty_old = 0;                                                             \
            faultDesc fault_desc(cycle, to_return.is_glob, to_return.struct_var, faultTrack::AUTOCORRECTED); \
            auto it = fault_list.find(to_return.meta.index);                                   \
            if (it != fault_list.end()) {                                                   \
                fault_desc.src = it->second.back().src;                                     \
                it->second.push_back(fault_desc);                                           \
