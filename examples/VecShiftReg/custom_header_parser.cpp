...
void header_injection(FILE * header_ptr, FILE * out_fp, char * header_file){
    ...
    char name_and_update[PREFIX_SIZE];  //cat-ed name and call to name_update
    ...
    while((line_len = getline(&line, &line_size, header_ptr) != -1)){
        ...
        if(strstr(line_backup, "void EVAL_") != NULL){
            ...
            //fprintf(out_fp, "%s.name_update_num(cur_prefix, \"%s\", a);}\n", token, token);
            ...
            token = strtok(line_backup, ".");
            token = trimspaces(token);
            //fprintf(out_fp, "%s.name_update_prefix(file_name, cur_prefix, \"%s\");\n", token, token);
    ...
    if(!skip_flag){
        ...
        while((line_len = getline(&line, &line_size, header_ptr) != -1)){
            ...
            if(strstr(line_backup, "l(b") != NULL){
                fprintf(out_fp, "%s", line_backup);
                fprintf(out_fp, "//--------\n");
            ...
            else if((search0 != NULL) && (token0 != NULL)){
                fprintf(out_fp, "%s", line);
                ...
                if((strstr(line_backup, "UInt<")!=NULL) && (strstr(line_backup_temp, ">(0x")==NULL)){
                    ...
                    if(name_and_update[1] != '('){
                        increment_str = std::to_string(increment);
                        strcat(name_and_update, increment_str.c_str());
                        strcat(name_and_update, ");");
                        fprintf(out_fp, "%s\n", name_and_update);
                        increment = increment + 1;
                    }
                    //search0 = trimspaces(search0);
                    //strncat(name_and_update, search0, (tok_len - 1));
                    //strcat(name_and_update, "\");");
                    //search0 = trimspaces(name_and_update);
                    //fprintf(out_fp, "%s\n", search0);
                    memset(name_and_update, 0, PREFIX_SIZE);
                }
    ...
    if(verbose_flag) printf("Done writing file, exiting...");
